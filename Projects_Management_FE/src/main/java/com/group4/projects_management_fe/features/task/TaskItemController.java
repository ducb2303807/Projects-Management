...
//    Callback được truyền từ TasksViewController → TaskGroupController → TaskItemController
    ...
    public void bindData(TaskResponseDTO task) {
        ...
        // Đổ màu Status (On going, Completed, v.v.)
        String status = task.getStatusName() != null ? task.getStatusName().toLowerCase().replace(" ", "-") : "todo";
        statusLabel.setText(task.getStatusName());
        statusLabel.getStyleClass().setAll("status-badge", "status-" + status);
    }

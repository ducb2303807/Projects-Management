...
class LookupServiceImplTest {
    ...
    class GetAllTests {
        ...
        void getAll_Fail_TypeNotRegistered() {
            // có một type mới trong Enum nhưng chưa code registry.put trong init
    ...
    class SaveOrUpdateTests {
        ...
        void create_Success() {
            LookupDTO input = new LookupDTO(null, "NEW", "Desc");
        ...
        void update_Success() {
            // Sử dụng TASK_STATUS vì trong init() nó được set editable = true
            LookupDTO input = new LookupDTO("1", "UPDATED", "Desc");
        ...
        void update_Fail_NotFound() {
            LookupDTO input = new LookupDTO("999", "NAME", "DESC");
            // Dùng TASK_STATUS để tránh bị chặn bởi quyền editable
        ...
        void update_Fail_SystemLocked() {
            // Trong hàm init(): APP_ROLE có editable = false
            LookupDTO input = new LookupDTO("1", "HACK", "Desc");

            // Mock check quyền (nếu logic check này được mở trong code của bạn)
            // Nếu code bạn đang throw BusinessException ở dòng 81, test này sẽ pass
    ...
    void testParseIdLogic() {
        LookupDTO dtoNumeric = new LookupDTO("123", "Name", "Desc");
